...
socket.on('connect', function(data){
    ...
    console.log(`game pin is ${quizPin}`);
    updatePinElement();
});

function updatePinElement() {
    console.log(`updating game pin to: ${quizPin}`)
    document.getElementById('quizPin').innerText =  ` Game Pin: ${quizPin}`;
}
...
function updateDatabase(){
    ...
    var quiz = {pin: quizPin, "name": name, "questions": questions};
...
socket.on('startGameFromCreator', function(data){
    window.location.href = "/createlobby?pin=" + data + "&mode=interactive";
});
